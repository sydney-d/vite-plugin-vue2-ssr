...
function createDescriptor(filename, source, { root, isProduction, sourceMap, compiler }) {
  ...
  const normalizedPath = slash$1(path.normalize(path.relative(root, filename)));
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (options.devServer) {
    if (filename.startsWith(options.root)) {
      assetUrlOptions = {
        base: (options.devServer.config.server?.origin ?? "") + options.devServer.config.base + slash$1(path.relative(options.root, path.dirname(filename)))
      };
    }
