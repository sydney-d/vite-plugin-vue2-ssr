...
function createDescriptor(filename, source, { root, isProduction, sourceMap, compiler }) {
  ...
  const normalizedPath = slash$1(path__default.normalize(path__default.relative(root, filename)));
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (options.devServer) {
    if (filename.startsWith(options.root)) {
      assetUrlOptions = {
        base: (options.devServer.config.server?.origin ?? "") + options.devServer.config.base + slash$1(path__default.relative(options.root, path__default.dirname(filename)))
      };
    }
...
async function transformMain(code, filename, options, pluginContext, ssr) {
  ...
  const moduleIdentifier = normalizePath(path__default.relative(options.root, filename));
  output.push(
    `/* normalize component */
  ...
  if (devToolsEnabled || devServer && !isProduction) {
    output.push(
      `__component__.options.__file = ${JSON.stringify(
        isProduction ? path__default.basename(filename) : filename
      )}`
    );
  }
